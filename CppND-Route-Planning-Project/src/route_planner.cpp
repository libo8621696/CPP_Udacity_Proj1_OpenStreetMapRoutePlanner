...
//   m_Model.path = ConstructFinalPath(end_node);
  ...
  // Expand nodes until you reach the goal. Use heuristic to prioritize what note to open first.
  while(open_list.size() > 0){
    // Select the best node to expore next.
    current_node = NextNode();
    // Check if thenode selected is the goal.
